...
class AudioSender:
    ...
    def __init__(self, host, port, audio_format=pyaudio.paInt16, channels=1, rate=44100, frame_chunk=4096):
        ...
        for i in range(0, numdevices):
                if (self.__audio.get_device_info_by_host_api_device_index(0, i).get('maxInputChannels')) > 0:
                    print("Input Device id ", i, " - ", self.__audio.get_device_info_by_host_api_device_index(0, i).get('name'))

        print("-------------------------------------------------------------")

        self.__input_device_index = int(input())
        print("recording via index "+str(self.__input_device_index))
    ...
    def __client_streaming(self):
        ...
        self.__stream = self.__audio.open(format=self.__audio_format, channels=self.__channels, rate=self.__rate, input=True, frames_per_buffer=self.__frame_chunk ,input_device_index=self.__input_device_index)
